...
for i in range(len(words)):
    ...
    if i + 1 < len(words):
        next_word_from_list = clean_token(words[i + 1])
    
    for word in word_from_list:
        for char in word:
            num_chars += 1

        if repeat_letter(word):
            repeats += 1
            for next_word in next_word_from_list:
                if end_start(word, next_word):
                    matches += 1
